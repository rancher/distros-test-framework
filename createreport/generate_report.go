...
func initCommand() *cobra.Command {
	upload, _ = strconv.ParseBool(os.Args[3])
...
func ProcessTestData(rowData []GoTestJSONRowData) ProcessedTestdata {
	...
	for _, r := range rowData {
		if r.Test != "" {
			...
			if strings.Contains(r.Output, "k3s rke2 test") {
...
// GenerateHTMLReport generates report in the form rke2/k3s_<OS>_acceptance_results.html
func GenerateHTMLReport(totalTestTime, testDate string, testSummary []TestOverview, testSuiteDetails map[string]TestSuiteDetails) {
	...
	htmlReport = strings.Split(fileName, ".")[0] + "_acceptance_results.html"
