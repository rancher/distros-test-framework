...
func main() {
	...
	if running, reason := isTestsRunning(); running {
		resources.LogLevel("info", "Tests already running (%s) - skipping this poll cycle", reason)
	...
	if rerunMsg == nil {
		resources.LogLevel("info", "No rerun request found in new messages - nothing to do")
		return
	}
...
func initializePoller() (state *rerunState, stateFile, slackToken string) {
	...
	if slackToken == "" {
		resources.LogLevel("error", "SLACK_TOKEN environment variable not set")
		os.Exit(1)
	}
	resources.LogLevel("info", "Slack token loaded (length: %d)", len(slackToken))

	stateFile = filepath.Join(getHomeDir(), "distros-test-framework", "report", ".rerun-state.json")
	resources.LogLevel("info", "State file: %s", stateFile)

	var err error
	state, err = loadState(stateFile)
	if err != nil {
		if os.IsNotExist(err) {
			resources.LogLevel("info", "No state file - nothing to poll")
		} else {
			resources.LogLevel("error", "Failed to load state: %v", err)
		}
		os.Exit(0)
	}
	resources.LogLevel("info", "Loaded state: thread=%s, channel=%s, last_ts=%s",
		state.ThreadTS, state.ChannelID, state.LastProcessedTS)
	resources.LogLevel("info", "Failed tests from last run: %v", state.FailedTests)
...
func findRerunRequest(state *rerunState, slackToken string) (rerunMsg *slackMessage, rerunTests string) {
	resources.LogLevel("info", "Fetching replies from Slack...")
	replies, err := fetchSlackReplies(slackToken, state.ChannelID, state.ThreadTS)
	if err != nil {
		resources.LogLevel("error", "Failed to fetch Slack replies: %v", err)
		os.Exit(1)
	}
	resources.LogLevel("info", "Fetched %d messages from thread", len(replies.Messages))
	...
	for i := len(replies.Messages) - 1; i >= 0; i-- {
		...
		if tests != "" {
			...
			if err != nil {
				resources.LogLevel("warn", "Failed to check channel membership for user %s: %v", msg.User, err)
				continue
			}
			if !isMember {
				resources.LogLevel("warn", "Unauthorized rerun request from user %s (not a channel member)", msg.User)
				continue
			}

			rerunMsg = &msg
			rerunTests = tests
			resources.LogLevel("info", "Found rerun request: ts=%s, user=%s, tests='%s'", msg.TS, msg.User, tests)

			break
		}
		resources.LogLevel("info", "Skipping non-rerun message: ts=%s, text='%s'", msg.TS, msg.Text)
	}
...
func processRerunRequest(state *rerunState, stateFile, slackToken string, rerunMsg *slackMessage, rerunTests string) {
	...
	if err := saveState(stateFile, state); err != nil {
		resources.LogLevel("error", "CRITICAL: Failed to save state: %v", err)
		resources.LogLevel("error", "Aborting to prevent duplicate triggers")
		os.Exit(1)
	}
	resources.LogLevel("info", "SAFETY: Updated last_processed_ts from %s to %s BEFORE processing", oldTS, rerunMsg.TS)

	// map "failed" to actual test names.
	if rerunTests == "failed" {
		if len(state.FailedTests) == 0 {
			resources.LogLevel("warn", "User requested 'rerun: failed' but no failed tests recorded")
		...
		resources.LogLevel("info", "Resolved 'failed' to: %s", rerunTests)
	}

	// check tests aren't running now.
	if running, reason := isTestsRunning(); running {
		resources.LogLevel("warn", "Tests started running between checks (%s) - aborting", reason)
		os.Exit(0)
	}

	if err := executeRerun(rerunTests, slackToken, state.ChannelID, state.ThreadTS, rerunMsg.User); err != nil {
		resources.LogLevel("error", "Rerun failed: %v", err)
		os.Exit(1)
	}
}
	...
	if data, err := os.ReadFile(pollLockFile); err == nil {
		...
		if pid, err := strconv.Atoi(pidStr); err == nil && pid > 0 {
			if processExists(pid) {
				resources.LogLevel("info", "Another poller instance running (PID %d) - exiting", pid)
				return false
			}
		}
		resources.LogLevel("warn", "Removing stale poller lock (PID was %s)", pidStr)
		_ = os.Remove(pollLockFile)
	}

	if err := os.WriteFile(pollLockFile, []byte(strconv.Itoa(os.Getpid())), 0o600); err != nil {
		resources.LogLevel("error", "Failed to create poller lock: %v", err)
		return false
	}
	resources.LogLevel("info", "Acquired poller lock (PID %d)", os.Getpid())
...
func releasePollerLock() {
	_ = os.Remove(pollLockFile)
	resources.LogLevel("info", "Released poller lock")
}
...
func isTestsRunning() (running bool, reason string) {
	if data, err := os.ReadFile(lockFile); err == nil {
		...
		if pid, err := strconv.Atoi(pidStr); err == nil && pid > 0 {
			...
			resources.LogLevel("warn", "Stale test lock file found (PID %d dead), removing", pid)
...
func executeRerun(tests, slackToken, channelID, threadTS, user string) error {
	resources.LogLevel("info", "\nSTARTING RERUN\n")
	resources.LogLevel("info", "  Tests: %s", tests)
	resources.LogLevel("info", "  Requested by: %s", user)
	resources.LogLevel("info", "  Thread: %s\n", threadTS)
	...
	if err := postToSlack(slackToken, channelID, threadTS, msg); err != nil {
		resources.LogLevel("warn", "Failed to post start message to Slack: %v", err)
	}

	runTestsScript := filepath.Join(getHomeDir(), "run_tests.sh")
	resources.LogLevel("info", "Using run_tests.sh at: %s", runTestsScript)
	...
	if err != nil {
		...
		resources.LogLevel("error", "Rerun FAILED after %v (exit code %d)", duration, exitCode)
	...
	resources.LogLevel("info", "Rerun COMPLETED successfully in %v", duration.Round(time.Second))
