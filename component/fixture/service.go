...
func TestServiceClusterIp(deployWorkload bool) {
	...
	getClusterIp := "kubectl get pods -l k8s-app=nginx-app-clusterip -n auto-clusterip " +
	...
	clusterip, port, _ := shared.FetchClusterIP("auto-clusterip","nginx-clusterip-svc")
	nodeExternalIP := shared.FetchNodeExternalIP()
	for _, ip := range nodeExternalIP {
		err = assert.ValidateOnNode(ip, "curl -sL --insecure http://"+clusterip+":"+port+"/name.html",
			"test-clusterip")
		if err != nil {
			GinkgoT().Errorf("%v", err)
		}
