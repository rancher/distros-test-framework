...
func (k *Client) WaitForNodesReady(minReadyNodes int) error {
	...
	resources.LogLevel("info", "Initial nodes ready/total: %d/%d", nodesReady, nodesTotal)

	if nodesReady >= minReadyNodes {
		resources.LogLevel("info", "Required number of nodes are already ready: %d/%d", nodesReady, nodesTotal)
		return nil
	}

	resources.LogLevel("info", "Waiting for nodes to become ready... (%d/%d ready)", nodesReady, nodesTotal)
	...
	for {
		...
		case event, ok := <-watcher.ResultChan():
			...
			if !nodePreviousReady && nodeCurrentReady {
				*nodesReady++
				resources.LogLevel("info", "Node %s became ready (%d/%d)", node.Name, *nodesReady, nodesTotal)
			} else if nodePreviousReady && !nodeCurrentReady {
				*nodesReady--
				resources.LogLevel("info", "Node %s is no longer ready (%d/%d)", node.Name, *nodesReady, nodesTotal)
			}

			if *nodesReady >= minReadyNodes {
				resources.LogLevel("info", "Required number of nodes are now ready: %d/%d", *nodesReady, nodesTotal)
				return nil
			}
		}
	}
