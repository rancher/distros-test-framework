...
func validate(exec func(string) (string, error), args ...string) error {
	if len(args) < 2 || len(args)%2 != 0 {
		return resources.ReturnLogError("should send even number of args")
	}
	...
	for i := 0; i < len(args); i++ {
		...
		if i+1 < len(args) {
			...
			i++
			if assert == "" || cmd == "" {
				return resources.ReturnLogError("should not send empty arg for assert:%s "+"and/or cmd:%s", assert, cmd)
			}

			err := runAssertion(cmd, assert, exec, ticker.C, timeout)
			if err != nil {
				return resources.ReturnLogError("error from runAssertion():\n %v\n", err)
			}
		}
	}
...
func runAssertion(
	...
	for {
		select {
		case <-timeout:
			timeoutErr := resources.ReturnLogError("timeout reached for command:\n%s\n"+
		...
		case <-ticker:
			...
			if err != nil {
				if retry == 0 || retry == 4 {
					resources.LogLevel("warn", "error from exec runAssertion: %v\n"+
						"Retrying command: %s\n on retry count: %d\n", err, cmd, retry)
				}
				if retry > 5 {
					return resources.ReturnLogError("error from exec runAssertion after 5 retries: %v\n", err)
				}
...
func ValidateOnNode(ip string, args ...string) error {
	exec := func(cmd string) (string, error) {
		return resources.RunCommandOnNode(cmd, ip)
	}
	return validate(exec, args...)
}
