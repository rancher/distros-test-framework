...
func (c Client) SpecReportTestResults(ctx context.Context, cluster *resources.Cluster, report *Report, reportSummary string) {
	...
	if err != nil {
		resources.LogLevel("error", "failed to validate Qase IDs: %w\n", err)
	}
...
func validateQaseIDs() (runID int32, tcID *int64, err error) {
	if projectID == "" {
		resources.LogLevel("error", "QASE_PROJECT_ID is not set")
	}

	if qaseRunID == "" {
		resources.LogLevel("error", "QASE_RUN_ID is not set")
	}
...
func formatOptionalConfigs(c *resources.Cluster) string {
	...
	if c.NumBastion > 0 {
		bastionInfo := []struct{ labelKey, value string }{
			{"Bastion Public IPv4", c.Bastion.PublicIPv4Addr},
			{"Bastion Public DNS", c.Bastion.PublicDNS},
		}
