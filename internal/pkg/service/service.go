...
func (ms *ManageService) ManageService(ip string, actions []ServiceAction) (string, error) {
	...
	for _, act := range actions {
		log.Infof("Running %s %s service on node: %s", act.Action, act.Service, ip)
		if act.Action == rotate {
			rotateErr := resources.CertRotate(act.Service, ip)
			if rotateErr != nil {
				log.Errorf("Error rotating certificate: %v", rotateErr)
		...
		cmd, buildCmdErr := CtlCmd(svcName, act.Action)
		if buildCmdErr != nil {
			return "", fmt.Errorf("command build failed for %s: %w", ip, buildCmdErr)
		}
		if act.ExplicitDelay > 0 {
			delay := act.ExplicitDelay * time.Second
			log.Infof("Waiting for %v before running systemctl %s %s on node: %s", delay, act.Action, svcName, ip)
			<-time.After(delay)
		}
		log.Debugf("Command: %s on node: %s", cmd, ip)
		output, execErr := ms.execute(cmd, act.Action, ip)
		if execErr != nil {
			log.Errorf("Error running command %s on %s: %v", cmd, ip, execErr)
			return "", fmt.Errorf("action %s failed on %s: %w", act.Action, ip, execErr)
		}
		if act.Action == status {
			log.Debugf("service %s output: \n %s", act.Action, output)
			return strings.TrimSpace(output), nil
		}
		log.Infof("Finished running systemctl %s %s on node: %s\n", act.Action, svcName, ip)
		if output != "" {
			log.Warnf("Output: %s", strings.TrimSpace(output))
		}
	}
...
func (ms *ManageService) execute(cmd, action, ip string) (string, error) {
	...
	if action == start || action == restart {
		result, err = resources.RunCommandOnNode(cmd, ip)
		if err != nil {
			log.Errorf("Error running command %s on %s: %v", cmd, ip, err)
	...
	retryErr := retry.Do(
		...
		retry.OnRetry(func(n uint, err error) {
			if n == 0 || n == ms.MaxRetries-1 {
				log.Warnf("Retry %d/%d failed for %s on %s: %v at (%s)",
...
// CtlCmd returns the systemctl command based on the action and service.
...
func CtlCmd(service, action string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("action value should be: start | stop | restart | status | enable")
	}
...
func productService(product, nodeType string) (string, error) {
	...
	if !ok {
		return "", fmt.Errorf("nodeType needs to be one of: server | agent")
	}
