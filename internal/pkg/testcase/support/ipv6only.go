...
func ConfigureIPv6OnlyNodes(cluster *driver.Cluster, awsClient *aws.Client) error {
	...
	for _, nodeIP := range nodeIPs {
		...
		go func(ip string) {
			...
			resources.LogLevel("info", "Copying configure.sh script on node: %s", ip)
			...
			resources.LogLevel("info", "Processing configure.sh on node: %s", ip)
			if err := processConfigureFile(cluster, awsClient, ip); err != nil {
				errChan <- resources.ReturnLogError("error configuring node: %v\n, err: %w", ip, err)
				return
			}

			resources.LogLevel("info", "Copying install script on node: %s", ip)
...
func copyInstallScript(cluster *driver.Cluster, ip string) (err error) {
	var script string
	cmd := fmt.Sprintf(
		"sudo chmod 400 /tmp/%v.pem && ", cluster.SSH.KeyName)
