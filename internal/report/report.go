...
func nodeSummaryData(c *driver.Cluster, data *summaryReportData) error {
	...
	unameOutput, err := resources.RunCommandOnNode("uname -r", c.ServerIPs[0])
	if err != nil {
		unameOutput = "Kernel version not found " + fmt.Sprintf("error: %v", err)
		data.summaryData.WriteString("\n" + "**Kernel Version**" + "\n" + unameOutput + "\n")

		resources.LogLevel("warn", "Kernel version not found on node %s", c.ServerIPs[0])
	}
...
func airgapNodeSummaryData(c *driver.Cluster, flags *customflag.FlagConfig, data *summaryReportData) error {
	...
	if unameErr != nil {
		...
		resources.LogLevel("warn", "Kernel version not found on node %s via bastion %s: %v",
			c.ServerIPs[0], c.Bastion.PublicIPv4Addr, unameErr)
	}
	unameOutput = strings.TrimSpace(unameOutput)
...
func remoteExec(keyName, user, serverIP, bastionIP, cmd string) (string, error) {
	...
	return resources.RunCommandOnNode(targetCmd, bastionIP)
}
...
func getSELinuxInfo(product, installMethod, nodeIP string) string {
	...
	cmdOutput := func(title, command string) {
		output, err := resources.RunCommandOnNode(command, nodeIP)
		if err != nil {
			resources.LogLevel("debug", "Failed to get %s: %v", title, err)
...
func getSplitRoleData(config *driver.Config, serverIps []string) string {
	...
	for _, ip := range serverIps {
		...
		configYaml, err := resources.RunCommandOnNode(cmd, ip)
		if err != nil {
			resources.LogLevel("warn", "Error retrieving config.yaml from node %s: %v", ip, err)
		...
		if err != nil {
			resources.LogLevel("warn", "Error determining role for node %s: %v", ip, err)
