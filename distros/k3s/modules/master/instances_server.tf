resource "aws_db_instance" "db" {
  count                  = (var.datastore == "etcd" || var.external_db == "" || var.external_db == "NULL" ? 0 : (var.external_db != "" && var.external_db != "aurora-mysql" ? 1 : 0))
...
resource "aws_rds_cluster" "db" {
  count                  = (var.external_db == "aurora-mysql" && var.datastore == "external_db" ? 1 : 0)
...
resource "aws_instance" "master" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo /tmp/install_k3s_master.sh ${var.node_os} ${var.create_lb ? aws_route53_record.aws_route53[0].fqdn : self.public_ip} ${var.install_mode} ${var.k3s_version} ${var.datastore == "external_db" ? var.datastore : "etcd"} ${self.public_ip} \"${data.template_file.test.rendered}\" \"${var.server_flags}\"  ${var.username} ${var.password}",
    ]
  }
