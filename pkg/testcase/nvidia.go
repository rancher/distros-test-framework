...
func validateNvidiaModule(ip string) {
	...
	cfg.DelayMultiplier = 1.0
	cfg.RetryableExitCodes = []int{1}
	...
	if !strings.Contains(out, "nvidia") ||
		!strings.Contains(out, "nvidia_uvm") ||
		!strings.Contains(out, "nvidia_drm") ||
		!strings.Contains(out, "nvidia_modeset") {

		shared.LogLevel("error", "NVIDIA modules not found in lsmod output:\n%s", out)
...
func validateNvidiaOperatorDeploy(nodeName string) {
	...
	retryErr := retry.Do(
		func() error {
			...
			if !strings.Contains(res, "\"nvidia.com/gpu.deploy.driver\":"+"\"pre-installed\"") {
				return errors.New("wrong label value for nvidia.com/gpu.deploy.driver")
			}

			if !strings.Contains(res, "nvidia.com/cuda.driver.major") && !strings.Contains(res, "nvidia.com/gpu.machine") {
				return errors.New("nvidia.com/cuda.driver.major or nvidia.com/gpu.machine label not found")
			}

			if !strings.Contains(res, "nvidia.com/gpu.count") {
				return errors.New("nvidia.com/gpu.count label not found")
			}
			if !strings.Contains(res, "nvidia.com/gpu.product") {
				return errors.New("nvidia.com/gpu.product label not found")
			}
