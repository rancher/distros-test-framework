...
func TestUpgradeReplaceNode(cluster *shared.Cluster, flags *customflag.FlagConfig) {
	...
	for i := 0; i < len(cluster.ServerIPs); i++ {
		serverNames = append(serverNames, fmt.Sprintf("%s-distros-server-replace-%d", resourceName, i+1))
	}
...
func nodeReplaceAgents(
	...
	for i := 0; i < len(cluster.AgentIPs); i++ {
		agentNames = append(agentNames, fmt.Sprintf("%s-distros-agent-replace-%d", resourceName, i+1))
	}
