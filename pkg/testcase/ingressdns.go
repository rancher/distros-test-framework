...
// 	for _, ip := range ingressIps {
// 		// Get the public IP for SSH connection (map internal to public IP)
// 		sshIP, err := shared.GetPublicIPForNode(ip)
// 		if err != nil {
// 			shared.LogLevel("warn", "Could not map internal IP %s to public IP, using internal IP: %v", ip, err)
// 			sshIP = ip // Fallback to internal IP
// 		}

// 		err = assert.CheckComponentCmdNode("curl -s --header host:foo1.bar.com"+
// 			" http://"+ip+"/name.html",
// 			sshIP, // Use public IP for SSH
// 			"test-ingress",
// 		)
// 	}
// 	Expect(err).NotTo(HaveOccurred(), err)
...
// 	// Get the public IP for the worker node (handle case where ExternalIP is <none>)
// 	var publicIp string
// 	if workerNodes[0].ExternalIP != "" && workerNodes[0].ExternalIP != "<none>" {
// 		publicIp = workerNodes[0].ExternalIP + ".nip.io"
// 	} else {
// 		// Fallback: get public IP from cluster configuration
// 		publicIP, err := shared.GetPublicIPForNode(workerNodes[0].InternalIP)
// 		Expect(err).NotTo(HaveOccurred(), "Failed to get public IP for worker node")
// 		publicIp = publicIP + ".nip.io"
// 	}
