...
func TestIngressRoute(applyWorkload, deleteWorkload bool, apiVersion string) {
	...
	if applyWorkload {
		...
		content, eee := os.ReadFile(originalFilePath)
		if eee != nil {
			Expect(eee).NotTo(HaveOccurred(), "failed to read file for ingressroute resource")
		}

		replacer := strings.NewReplacer("$YOURDNS", publicIp, "$APIVERSION", apiVersion)
		newContent := replacer.Replace(string(content))
		ewew := os.WriteFile(newFilePath, []byte(newContent), 0644)
		if ewew != nil {
			Expect(ewew).NotTo(HaveOccurred(),
