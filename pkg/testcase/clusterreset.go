...
func TestClusterReset() {
	...
	productLocationCmd := fmt.Sprintf("sudo find / -type f -executable -name %s 2> /dev/null | grep -v data | sed 1q", cluster.Config.Product)
...
func killall(cluster *factory.Cluster) {
	for i := len(cluster.ServerIPs) - 1; i > 0; i-- {
		killallLocationCmd := fmt.Sprintf("sudo find / -type f -executable -name %s-killall.sh 2> /dev/null | grep -v data  | sed 1q", cluster.Config.Product)
