...
func checkResults(results string) error {
	...
	shared.LogLevel("info", "rerunning failed tests")
...
func rerunFailedTests(testResultTar string) error {
	cmd := fmt.Sprintf("sonobuoy run --rerun-failed=%s --kubeconfig=%s --kubernetes-version=%s",
		testResultTar, shared.KubeConfigFile, shared.ExtractKubeImageVersion())
