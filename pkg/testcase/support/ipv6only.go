...
func ConfigureIPv6OnlyNodes(cluster *shared.Cluster, awsClient *aws.Client) error {
	...
	for idx, nodeIP := range nodeIPs {
		wg.Add(1)
		// Pass idx and nodeIP to the goroutine to prevent closure issues
		go func(nodeIndex int, ip string) {
			defer wg.Done()

			if err := configureNodeAssets(cluster, awsClient, ip, nodeIndex); err != nil {
				// Log the detailed failure and send the error to the channel
...
func getKubeconfigSourceIP(cluster *shared.Cluster) (string, error) {
	if !cluster.Config.SplitRoles.Enabled {
		// Non-split roles: always use the first server.
	...
	// Split roles: find the server configured for the 'control-plane' role.
	...
	for _, serverIP := range cluster.ServerIPs {
		...
		// NOTE: Assuming CmdForPrivateNode returns empty string on shell non-zero exit/no match (as in original logic)
	...
	// Fallback: If no explicit control-plane role is found, use the first server.
