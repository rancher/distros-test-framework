...
func TestKillAllUninstall(cluster *shared.Cluster, cfg *config.Env) {
	...
	dirErr := exportBinDirs(append(cluster.ServerIPs, cluster.AgentIPs...),
		cluster.Config.Product, "crictl", "kubectl", "ctr")
...
func exportBinDirs(ips []string, binaries ...string) error {
	...
	return shared.ExportEnvNode(ips, envVars, pathDirs, "bin_paths.sh")
}
