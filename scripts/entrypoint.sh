...
[ -n "$K3S_CHANNEL" ] && sed -i "s/install_channel\s*=\s*.*/install_channel = \"$K3S_CHANNEL\"/" "$CONFIG_PATH"
[ -n "$RKE2_CHANNEL" ] && sed -i "s/install_channel\s*=\s*.*/install_channel = \"$RKE2_CHANNEL\"/" "$CONFIG_PATH"
