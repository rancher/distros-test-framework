...
func copyAssets(cluster *Cluster, ip string) (err error) {
	cmd := fmt.Sprintf(
		"sudo chmod 400 /tmp/%v.pem && ", cluster.Aws.EC2Config.KeyName)
	switch cluster.Config.Product {
	case "rke2":
		cmd += fmt.Sprintf(
			"sudo %v -r artifacts %v@%v:~/ && ",
			ssPrefix("scp", cluster.Aws.EC2Config.KeyName),
			cluster.Aws.EC2Config.AwsUser, ip)
	case "k3s":
		cmd += fmt.Sprintf(
			"sudo %v %v* %v@%v:~/ && ",
			ssPrefix("scp", cluster.Aws.EC2Config.KeyName),
...
func CmdForPrivateNode(cluster *Cluster, cmd, ip string) (res string, err error) {
	serverCmd := fmt.Sprintf(
		"%v %v@%v '%v'",
		ssPrefix("ssh", cluster.Aws.EC2Config.KeyName),
		cluster.Aws.EC2Config.AwsUser, ip, cmd)
