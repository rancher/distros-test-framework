...
var _ = Describe("VersionTemplate Upgrade:", func() {
	...
	It("Install helm and validate traefik repo", func() {
		...
		Expect(err).NotTo(HaveOccurred(), "Traefik repo not added")
	})
		...
		template.VersionTemplate(template.VersionTestTemplate{
			TestCombination: &template.RunCmd{
				Run: []template.TestMap{
					{
						Cmd:                  "helm list --all-namespaces,kubectl exec -n dnsutils -t dnsutils : -- nslookup kubernetes.default",
